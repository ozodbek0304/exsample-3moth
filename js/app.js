...
let newPrice = products.sort(function (a, b) {
  return b.price - a.price;
})

for (let i = 0; i < 5; i++){
  price1.push(newPrice[i])
}
for (let i = 15; i < 19; i++){
  price2.push(newPrice[i]);
}

let newOrta = products.filter(item => {
  return item.price > 60 && item.price < 700;
})
console.log(price2);
...
newOrta.forEach(item => {
  let boxs1 = document.querySelector(".boxs1");
  boxs1.style.transform = "translateY(-150px)";
  ...
  card1.appendChild(hover);
  boxs1.appendChild(card1);
 
});
price2.forEach(item => {
    let boxs2 = document.querySelector(".boxs2");
   let elp = document.createElement("p");
  let elp1 = document.createElement("p");
  let icon = document.createElement("i");
  icon.className = "fa-solid fa-star mt-1 ";
